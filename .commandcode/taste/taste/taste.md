...
- Product/collection photos must never be cropped — but the grid must stay visually uniform. When per-image native ratios made cards uneven ("biri büyük biri küçük"), the user rejected that as messy ("çok karışık oldu"); the accepted solution is fixed uniform frames (4:5) with `object-fit: contain` on a neutral linen background, applied consistently to cards, hero, atelier, and modal. Confidence: 0.8
